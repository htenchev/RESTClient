...
protocol APIRequest {
    func load(urlSession: URLSession, requestable: Requestable, completion: @escaping (OperationResult?, RequestError?) -> Void)
...
extension APIRequest {
    func load(urlSession: URLSession, requestable: Requestable, completion: @escaping (OperationResult?, RequestError?) -> Void) {
        guard let urlRequest = requestable.urlRequest else {
            completion(nil, RequestError(description: "Bad url request"))
            return
        }

        guard let url = urlRequest.url else {
            completion(nil, RequestError(description: "Bad url"))
            return
        }
        ...
        (data: Data?, response: URLResponse?, error: Error?) -> Void in
            guard let mydata = data else {
                completion(nil, RequestError(description: "Cannot parse data."))
                return
            }
...
struct RESTRequest : APIRequest {
    ...
    func modelFromData(data: Data) -> OperationResult? {
        ...
        guard let apiMethod = self.requestable as? API else {
            return nil //.error("Bad requestable, could not cast to API")
        }
    ...
    func execute(completion: @escaping (OperationResult?, RequestError?) -> Void) {
    ...
    private static let urlSession = { () -> URLSession in 
        ...
        config.httpAdditionalHeaders = [Constants.contentType: Constants.contentTypeJSON]
